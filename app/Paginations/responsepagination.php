...
class responsepagination {
	...
	function getPrevNext($count,$href) {
		...
		if($pages>1) {
			...
			if($_GET["page"] < $pages)
				//$output = $output . '<a  class="btn btn-success" onclick="getresult(\'' . $href . ($_GET["page"]+1) . '\')" >Submit</a>';
				$output = $output . '<a  class="btn btn-success" onclick="updatedata(\'' . $href . ($_GET["page"]+1) . '\')" >Submit</a>';
			else				
				$output = $output . '<span class="btn btn-success" onclick="updatedata(\'' . $href . ($_GET["page"]+1) . '\')">Submit</span>';
