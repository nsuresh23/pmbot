			...
			<table id="usersreport" class="stripe row-border order-column" cellspacing="0" width="100%">
					<thead>
						<tr>
							...
							<th>Books/Journals</th>
							<th>Signal Filenames - CL</th>  
							<th>Car Order Name</th>  														
							<th>Unit Count</th>
							<th>Manifestation</th>
							<th>Content Provider Name</th>								
							<th>Reasons</th>	
							<th>Suggested by (supplier name/Elsevier requester name)</th>	
							<th>Source ID</th>	
							<th>Title</th>	
							<th>Year</th>	
							<th>Vol</th>	
						...
						@if (count($result))
							@foreach ($result as $k => $results)
								<tr>             						
									...
									<td @if ($k % 2 == 0) class='bgcolor' @endif>-</td>									
									<td @if ($k % 2 == 0) class='bgcolor' @endif>{{ $results->srmanifest }}</td>
									<td @if ($k % 2 == 0) class='bgcolor' @endif>{{ $results->srcontentprovider }}</td>		
									<td @if ($k % 2 == 0) class='bgcolor' @endif>{{ $results->remark }}</td>
									<td @if ($k % 2 == 0) class='bgcolor' @endif>-</td>									
           ...
           <script type="text/javascript" class="init">
			...
			function export_excel(event,listcount)
			{
				if(listcount > 0){
					...
					if(passparam!='') 
						passparam = '?'+passparam;				
					window.location.href ="{{url('reports/dailyemptysignal_exportxls')}}"+passparam;
				} else {
