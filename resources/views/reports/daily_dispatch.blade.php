						...
						@if (count($result))
							@foreach ($result as $k => $results)
								<tr>             															
									...
									<td @if ($k % 2 == 0) class='bgcolor' @endif>{{ $results->dispatchdate != '' ? date('d-M-y', strtotime($results->dispatchdate)) : "" }}</td>	<td @if ($k % 2 == 0) class='bgcolor' @endif>{{ $results->publisher }}</td>		
									<td @if ($k % 2 == 0) class='bgcolor' @endif>XML</td>		
									<td @if ($k % 2 == 0) class='bgcolor' @endif>Regular</td>		
           ...
           <script type="text/javascript" class="init">
			...
			function export_excel(event,listcount)
			{
				if(listcount > 0){
					...
					if(passparam!='') 
						passparam = '?'+passparam;				
					window.location.href ="{{url('reports/dailydispatch_exportxls')}}"+passparam;
				} else {
