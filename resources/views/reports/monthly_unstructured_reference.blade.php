			...
			<table id="usersreport" class="stripe row-border order-column" cellspacing="0" width="100%">
					<thead>
						<tr>
							...
							<th>No of Structured Reference</th>
							<th>No of Unstructured Reference</th>	
						</tr>
					</thead>
					<tbody>
						@if (count($result))
							@foreach ($result as $k => $results)
								<tr>             						
									...
									<td @if ($k % 2 == 0) class='bgcolor' @endif>{{ $results->number_of_reference != '' ? $results->number_of_reference  : "-" }}</td>									
									<td @if ($k % 2 == 0) class='bgcolor' @endif>{{ $results->number_of_reference - $results->unstructured_count }}</td>
									<td @if ($k % 2 == 0) class='bgcolor' @endif>{{ $results->unstructured_count != '' ? $results->unstructured_count  : "-" }}</td>												
								</tr>								
							@endforeach
						@else
           ...
           <script type="text/javascript" class="init">
			...
			function export_excel(event,listcount)
			{
				if(listcount > 0){
					...
					if(passparam!='') 
						passparam = '?'+passparam;				
					window.location.href ="{{url('reports/monthlyunstructuredreference_exportxls')}}"+passparam;
				} else{
