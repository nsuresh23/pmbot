...
<!--  @extends('layouts.app') -->
	<style type="text/css" class="init">
		...
		#individualjobsreport_filter,#individualjobshistory_filter,#individualticketreport_filter,#jobshistory_filter,#individualquerylist_filter,#ticketstatusreport_filter,#ticketstatusreport_length{display:none;}
...
<?php //echo route('editgroup'); ?>
      <div class="box">
		...
		<form class="form-horizontal" name="searchform" id="searchform" method="POST" action="{{url('reports/jobs_report/detailed_report')}}">
            {{ csrf_field() }}
			<div class='searchbox'>
				...
				<div class="col-md-2 norightpadding">
					...
					<div style='margin-top:30px;' class="col-md-1" >                            
						<label>Job ID:</label>&nbsp;
						<input type='radio' name='searchfield' id='searchfield' <?php if($searchfield == 'job_id'){ ?> checked <?php } ?> value='job_id'>
                    </div>	
					<div style='margin-top:30px;' class="col-md-1 nopadding" >                            
						<label>Order ID:</label>&nbsp;
						<input type='radio' name='searchfield' id='searchfield' <?php if($searchfield == 'orderid'){ ?> checked <?php } ?> value='orderid'>
                    </div>						
					<div class="col-md-2" style='margin-top:30px;'  >                            
						 <input type='text' id='searchtext' class='form-control' name='searchtext' value='<?php echo $searchtext; ?>'> 
                    </div>					
					

					<div class="col-md-1 form-group" style="text-align: center;margin-top:5px;' ">
						<label>&nbsp;</label>	
						...
						@if (count($jobresult))
							@foreach ($jobresult as $k => $jobticketstatusresults)
								<tr>             						
									...
									<td @if ($k % 2 == 0) class='bgcolor' @endif>{{ $jobticketstatusresults->remarks }}</td>	
									<td @if ($k % 2 == 0) class='bgcolor' @endif>
									<?php
										echo wordwrap($jobticketstatusresults->error_info,100,"<br>\n");
									?>
									</td>									
									<td @if ($k % 2 == 0) class='bgcolor' @endif>
						...
						@if (count($ticketresult))
							@foreach ($ticketresult as $k => $ticketticketstatusresults)
								<tr>             						
									...
									<td>
											<?php
												echo gmdate("H:i:s",  $ticketticketstatusresults->conversion_time);
											?>											
									</td>										
						...
						@if (count($queryresult))
							@foreach ($queryresult as $k => $queryticketstatusresults)
								...
								<tr>             						
									...
									<?php echo base64_decode($queryticketstatusresults->query); ?>
