<meta charset="UTF-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no" />
{{-- <title>PMBot</title> --}}
<title>{{ __('general.appTitle') }}</title>
<!-- Site Title -->
{{-- <title>{{ $title }}</title> //dynamic title --}}
<meta name="description" content="PMBot." />
<meta name="keywords" content="PMBot" />
<meta name="author" content="spi" />

<!-- CSRF Token -->
<meta name="csrf-token" content="{{ csrf_token() }}">

<!-- Favicon -->
<link rel="shortcut icon" href="favicon.ico">
<link rel="icon" href="favicon.ico" type="image/x-icon">

<!-- Styles -->
{{-- <link href="{{ asset('css/app.css') }}" rel="stylesheet"> --}}

<!-- vector map CSS -->
<link href="{{ asset('css/custom/css/style.css') }}" rel="stylesheet">

<link href="{{ asset('js/custom/vendors/bower_components/bootstrap/dist/css/bootstrap.min.css') }}" rel="stylesheet" type="text/css">
<!-- Custom CSS -->
<link href="{{ asset('js/custom/vendors/bower_components/jasny-bootstrap/dist/css/jasny-bootstrap.min.css') }}" rel="stylesheet" type="text/css">

<!-- Data table CSS -->
<link href="{{ asset('js/custom/vendors/bower_components/datatables/media/css/jquery.dataTables.min.css') }}" rel="stylesheet" type="text/css" >

<!-- Toast CSS -->
{{-- <link href="{{ asset('js/custom/vendors/bower_components/jquery-toast-plugin/dist/jquery.toast.min.css') }}" rel="stylesheet" type="text/css"> --}}

