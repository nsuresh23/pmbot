(function() {
  ...
  //constants
  var IMAGE_DELETE =  'http://172.24.182.52/pmbot_v/v4/public/assets/src/img/icono_eliminar.png',
  IMAGE_DELETE_OVER = 'http://172.24.182.52/pmbot_v/v4/public/assets/src/img/papelera_over.png',
  SHARED_ICON = 'http://172.24.182.52/pmbot_v/v4/public/assets/src/img/shared-icon.png';
  ...
  Annotator.Plugin.AnnotatorViewer = (function(_super) {
    ...
    */
    ...
    AnnotatorViewer.prototype.onSavePanel = function(event) {
      ...
      var textarea = $('li#annotation-'+current_annotation.annotationid).find("#textarea-"+current_annotation.annotationid);
      if (typeof(this.annotator.plugins.RichEditor)!='undefined') {
    ...
    AnnotatorViewer.prototype.onCancelPanel = function(event) {
      ...
      if (typeof(this.annotator.plugins.RichEditor)!='undefined') {
        ...
        var textAreaEditor = $('li#annotation-'+current_annotation.annotationid + ' > .annotator-marginviewer-text');
       
        textAreaEditor.replaceWith(anotacio_capa);
      } else {
        var textarea = $('li#annotation-'+current_annotation.annotationid).find('textarea.panelTextArea');
        this.normalEditor(current_annotation,textarea);
      }
    ...
    AnnotatorViewer.prototype.normalEditor = function(annotation,editableTextArea) {
      var buttons = $('li#annotation-'+annotation.annotationid).find('div.annotator-textarea-controls');
    ...
    AnnotatorViewer.prototype.onAnnotationUpdated = function(annotation) {
      ...
      $( "#annotation-"+annotation.annotationid ).html( this.mascaraAnnotation(annotation) );       
    };
    ...
    AnnotatorViewer.prototype.mascaraAnnotation = function(annotation) {
	  ...
	  if(annotation.userid != null){
		  var  sectionAnnotation = annotation.userid.split(",");
		   $.each(sectionAnnotation,function(i){
			pmassigneduser +='<span style="color:#FFF; float:right; margin-right:5px;" class="label label-info">'+sectionAnnotation[i]+'</span>';
		});
		
	  } else {
		 $.each(annotation.section, function(key, value) { 
	 		 pmassigneduser +='<span style="color:#FFF; float:right; margin-right:5px;" class="label label-info">'+value+'</span>';
    	}); 
	  }
	 if(notesAnnotation ==''){
	 ...
	 if(annotation.group_ID === null || (typeof annotation.group_ID === "undefined")){
	  ...
	  var annotationstatus = '<span class="'+badgelabel+'" style="color:#FFF;">'+annotation.status+'</span>';
      var annotation_layer =  '<div class="annotator-marginviewer-text" ><div class="'+anotation_color+' anotator_color_box"><label  style="word-wrap:break-word">'+annotationgroupcheckbox+''+  notesAnnotation  + '</label>'+pmassigneduser+'</div></div>';
      annotation_layer += '<div class="annotator-marginviewer-quote">'+ annotation.quote + '</div>';
	  
	  annotation_layer += '</div><div class="annotator-marginviewer-date">'+ $.format.date(annotation.updated_at, "dd/MM/yyyy HH:mm:ss") + '</div><div class="annotator-marginviewer-footer">'+annotationstatus+shared_annotation+''+delete_icon+'';
	  ...
	  //alert(annotation.group_ID);
	   ...
	   if(annotation.group_ID === null || (typeof annotation.group_ID === "undefined")){
		  ...
		  // alert('notcomes');
		 annotation_layer +='  <span class="btn btn-primary btn-xs">'+annotation.group_ID+'</span></div>';  
	   }
    ...
    AnnotatorViewer.prototype.createAnnotationPanel = function(annotation) {   
		...
		if(womatRefID !=''){
			var wjobid = '<a class="list-group-item" href="#"><span class="badge badge-info">'+womatRefID+'</span> ISBN: </a>';
		} else {
			var wjobid = '';
		}
      ...
      var annotation_layer =  '<div  class="annotations-list-uoc '+annoclass+'" style="background-color:#F1F2F2; -moz-box-shadow: -5px -5px 5px #888; -webkit-box-shadow: -5px -5px 5px #888; box-shadow: -5px -5px 5px #888;"><div id="annotations-panel"><span class="rotate" title="'+ i18n_dict.view_annotations +' '+ i18n_dict.pdf_resum +'" style="padding:5px;background-color:#F1F2F2;  position: absolute; top:20em;left: -50px; width: 155px; height: 110px;cursor:pointer">'+ i18n_dict.view_annotations +'<span class="label-counter" style="padding:0.2em 0.3em;float:right" id="count-anotations">0</span></span></div><form><div id="anotacions-uoc-panel" style="height:80%"><ul class="container-anotacions"><li></li><li class="filter-panel">'+checboxes+'</li></ul></div></form></div>';
...
//      }
      ...
      anotacioObject = $(annotation_layer).appendTo('.container-anotacions').click(function(event) {
          ...
          if ($element.length) {
            ...
            $('html, body').animate({
              scrollTop: $("#"+annotation_reference).offset().top - (viewPanelHeight/6)
            }, 20000);
          } 
      })
      .mouseover(function() {
